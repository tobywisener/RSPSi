...
public final class MapRegion {
	...
	private static Sprite resolveMinimapFunctionSprite(ObjectDefinition definition) {
		if (definition.getAreaId() >= 0 && RSAreaLoader.instance != null) {
			RSArea area = RSAreaLoader.get(definition.getAreaId());
			if (area != null) {
				int spriteId = area.getSpriteId();
				if (spriteId >= 0) {
					return Client.getSingleton().getCache().getSprite(spriteId);
				}
			}
		}

		int minimapFunction = definition.getMinimapFunction();
		if (minimapFunction >= 0 && minimapFunction < Client.mapFunctions.length) {
			return Client.mapFunctions[minimapFunction];
		}
		return null;
	}

	private static Mesh mapIconPlaceholderTemplate;

	private static Renderable createMapIconPlaceholderRenderable() {
		if (mapIconPlaceholderTemplate == null) {
			Mesh marker = Mesh.EMPTY_MODEL.copy();
			marker.vertexCount = 8;
			marker.vertexX = new int[] { -20, 20, 20, -20, -20, 20, 20, -20 };
			marker.vertexY = new int[] { 0, 0, 0, 0, -84, -84, -84, -84 };
			marker.vertexZ = new int[] { -20, -20, 20, 20, -20, -20, 20, 20 };

			marker.triangleCount = 12;
			marker.faceIndices1 = new int[] { 0, 0, 4, 4, 0, 0, 1, 1, 2, 2, 3, 3 };
			marker.faceIndices2 = new int[] { 1, 2, 6, 5, 3, 7, 2, 6, 3, 7, 0, 4 };
			marker.faceIndices3 = new int[] { 2, 3, 7, 6, 7, 4, 6, 5, 7, 6, 4, 7 };

			int markerColour = ColourUtils.toHsl(18, 255, 180);
			marker.triangleColors = new int[marker.triangleCount];
			Arrays.fill(marker.triangleColors, markerColour);
			marker.faceTransparencies = new int[marker.triangleCount];
			marker.fitsOnSingleSquare = true;

			marker.computeBounds();
			marker.light(64, 768, -50, -10, -50, true);
			marker.computeSphericalBounds();
			mapIconPlaceholderTemplate = marker;
		}
		return mapIconPlaceholderTemplate.copy();
	}

	private static boolean isMapIconOnlyDecoration(ObjectDefinition definition, Renderable renderable) {
		boolean hasMapIconMetadata = definition.getAreaId() >= 0 || definition.getMinimapFunction() >= 0;
		int[] modelIds = definition.getModelIds();
		if (modelIds == null || modelIds.length == 0) {
			return hasMapIconMetadata;
		}

		int primaryModelId = modelIds[0];
		if (primaryModelId == 111 || primaryModelId == 1105) {
			return true;
		}

		int[] modelTypes = definition.getModelTypes();
		if (modelTypes != null && modelTypes.length > 0 && modelTypes[0] != 22) {
			return false;
		}

		return hasMapIconMetadata && renderable == null;
	}
	...
	public static int perlinNoise(int x, int y) {
		int n = x + y * 57;
		n = n << 13 ^ n;
	...
	private void decodeMapDataLegacy317(Buffer buffer, int x, int y, int z, int regionX, int regionY, int orientation) {
		if (x >= 0 && x < width && y >= 0 && y < length) {
			tileFlags[z][x][y] = 0;
			overlays[z][x][y] = 0;
			overlayShapes[z][x][y] = 0;
	...
	private void unpackTilesInternal(byte[] data, int dX, int dY, int regionX, int regionY, boolean useUShortEncoding) {
		Buffer buffer = new Buffer(data);
		for (int z = 0; z < 4; z++) {
			for (int localX = 0; localX < 64; localX++) {
				for (int localY = 0; localY < 64; localY++) {
					if (useUShortEncoding) {
						decodeMapData(buffer, localX + dX, localY + dY, z, regionX, regionY, 0);
					} else {
						decodeMapDataLegacy317(buffer, localX + dX, localY + dY, z, regionX, regionY, 0);
					}
				}
			}
		}
	...
	public final void unpackTiles(byte[] data, int dX, int dY, int regionX, int regionY) {
		try {
			unpackTilesInternal(data, dX, dY, regionX, regionY, true);
		} catch (ArrayIndexOutOfBoundsException ex) {
			System.out.println("Terrain decode fallback to 317 byte format for region " + regionX + "," + regionY
					+ " (data length=" + data.length + ")");
			unpackTilesInternal(data, dX, dY, regionX, regionY, false);
		}
							...
							if (underlay > 0 || overlayFloorId > 0) {
								...
								if (overlayFloorId == 0) {
									...
									if (overlayFloor == null) {
										overlayFloor = FloorDefinitionLoader.getOverlay(0);
									}
									if (overlayFloor == null) {
										overlayFloor = new Floor();
										overlayFloor.generateHsl();
									}
    ...
    public final ObjectKey spawnObjectToWorld(SceneGraph scene, int id, int x, int y, int z, int type, int orientation,
        ...
        if (type == 22) {
            ...
            Sprite minimapFunctionSprite = resolveMinimapFunctionSprite(definition);
            boolean mapIconOnlyDecoration = isMapIconOnlyDecoration(definition, object);
            if (mapIconOnlyDecoration) {
                // Force a visible marker for mapicon-only decorations.
                object = createMapIconPlaceholderRenderable();
            }

            GroundDecoration deco = scene.addFloorDecoration(x, y, z, object, objectKey, mean, temporary);

            if (deco != null && mapIconOnlyDecoration && minimapFunctionSprite != null) {
                deco.setMinimapFunction(minimapFunctionSprite);
            }
            ...
            if (object == null && isMapIconOnlyDecoration(definition, null)) {
                object = createMapIconPlaceholderRenderable();
            }
